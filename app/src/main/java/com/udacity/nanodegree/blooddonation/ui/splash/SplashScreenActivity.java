    ...
    public void run() {
      if (!isFinishing()) {
        ...
        if (FirebaseAuth.getInstance().getCurrentUser() == null) {
          ...
          if (Injection.getSharedPreference()
              .getBoolean(SharedPrefConstants.IS_USER_DETAILS_ENTERED)) {
            intent = new Intent(SplashScreenActivity.this, HomeActivity.class);
          } else {
            intent = new Intent(SplashScreenActivity.this, UserDetailActivity.class);
            intent.putExtra(UserPhoneNumber.EXTRA_USER_PHONE_NUMBER,
                UserPhoneNumber.fromSharedPreferences(Injection.getSharedPreference()));
          }
        }
        finish();
