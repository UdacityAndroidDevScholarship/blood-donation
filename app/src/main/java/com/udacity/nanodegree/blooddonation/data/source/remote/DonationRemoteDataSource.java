...
public class DonationRemoteDataSource implements DonationDataSource {
    ...
    @Override
    public void saveDonorDetails(String uid, String bgp, GeoLocation geoLocation,
                                 RequestDialogPresenter.ISaveDonorDetails iSaveDonorDetails) {


        //DatabaseReference databaseReference = mFirebaseDatabase.getReference().child(FireBaseConstants.DONOR).child(bgp);
        DatabaseReference databaseReference = mFirebaseDatabase.getReference().child(FireBaseConstants.DONOR);
